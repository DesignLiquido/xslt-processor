...
describe('xsl:choose', () => {
    ...
    /**
     * Test for issue #107 - Testing xsl:choose with complex nested conditions
     */
    it('https://github.com/DesignLiquido/xslt-processor/issues/107', async () => {
        ...
        const xsltSource = `<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
            ...
            <xsl:template match="/">
                ...
                <xsl:choose>
                    <xsl:when test="root/type = 'pzn'">pzn</xsl:when>
                    <xsl:when test="root/type = 'wirkstoff'">wirkstoff</xsl:when>
                    <xsl:when test="root/type = 'freitext'">freitext</xsl:when>
                    <xsl:when test="root/type = 'rezeptur'">rezeptur</xsl:when>
        ...
        assert.equal(outputXml.trim(), 'Type=wirkstoff');
    });
});
